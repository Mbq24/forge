...
class stockPred:
    ...
    def LSTM_model(self, input_data):
        ...
        model.add(tf.keras.layers.LSTM(self.neurons, input_shape=(
            input_data.shape[1], input_data.shape[2]), return_sequences=True))
        model.add(tf.keras.layers.Dropout(self.dropout))
        model.add(tf.keras.layers.LSTM(self.neurons, return_sequences=True))
        model.add(tf.keras.layers.Dropout(self.dropout))
        model.add(tf.keras.layers.LSTM(self.neurons))
        model.add(tf.keras.layers.Dropout(self.dropout))
        model.add(tf.keras.layers.Dense(units=self.output_size))
        model.add(tf.keras.layers.Activation(self.activ_func))
    ...
    def predictModel(self):
        logger.info("Getting Bitcoin Bar Data")
        # get all data
        df = self.getAllData()
